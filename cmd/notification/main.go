// Package main es el entrypoint del BC Notification.
...
func run() error {
	...
	log.Info("starting notification BC",
		slog.String("version", cfg.OTEL.ServiceVersion),
		slog.String("env", cfg.OTEL.Environment),
		slog.String("terminal_gateway_addr", cfg.GRPCClient.TerminalGatewayAddr),
		slog.Duration("webhook_timeout", cfg.Webhook.Timeout),
		slog.Duration("retry_job_interval", cfg.Retry.JobInterval),
	)
	...
	log.Info("notification BC ready — waiting for messages",
	...
	log.Info("notification BC stopped cleanly")
	return nil
}
