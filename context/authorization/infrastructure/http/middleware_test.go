...
func TestRecoverMiddleware_RecoversFromStringPanic(t *testing.T) {
	...
	// No debe propagar el panic al test — eso es justamente lo que valida este caso.
	handler.ServeHTTP(rec, req)
	...
	if body["message"] != "an unexpected error occurred" {
		t.Errorf("body[message] = %q, want %q", body["message"], "an unexpected error occurred")
	}
}
...
func TestRecoverMiddleware_RecoversFromNilPanic(t *testing.T) {
	// panic(nil) es un caso límite del lenguaje: recover() puede devolver un
	// *runtime.PanicNilError (Go 1.21+) en vez de nil — igualmente debe
	// entrar a la rama de recuperación.
	next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		panic(nil)
	})
	handler := recoverMiddleware(next)
